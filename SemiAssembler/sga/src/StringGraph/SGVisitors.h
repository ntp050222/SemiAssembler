...
//////////////// yuhan start ////////////////////////////////////////////////////////////////
// Visit each node, writing it to a file as a fasta record
struct SGMPFastaVisitor
{
    // constructor
    SGMPFastaVisitor(std::string filename) : m_fileHandle(filename.c_str(), std::ios::out | std::ios::app) {}
    ~SGMPFastaVisitor() { m_fileHandle.close(); }
...
struct SGMaxOverlapVisitor
{
    SGMaxOverlapVisitor(int overlapLen_cutoff) : m_overlapLen_cutoff(overlapLen_cutoff) {}

    void previsit(StringGraph* pGraph);
    bool visit(StringGraph* pGraph, Vertex* pVertex);
    void postvisit(StringGraph*);

    int m_overlapLen_cutoff;
};
