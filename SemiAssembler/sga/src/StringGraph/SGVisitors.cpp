...
//////////////// yuhan start ////////////////////////////////////////////////////////////////
//////////////// yuhan start ////////////////////////////////////////////////////////////////
bool SGMPFastaVisitor::visit(StringGraph* /*pGraph*/, Vertex* pVertex)
{
    m_fileHandle << ">" << pVertex->getID() << " " <<  pVertex->getSeq().length() 
                 << " " << 0 << "\n";
    m_fileHandle << pVertex->getSeq() << "\n";
...
/*****************************************CYC CODE****************************************/
//choose a maximal overlap edge,
//or keep all edges whcih overlap length are greater than cutoff(threashold)
//

void SGMaxOverlapVisitor::previsit(StringGraph*)
{

}

bool SGMaxOverlapVisitor::visit(StringGraph* /*pGraph*/, Vertex* pX)
{
    bool change = false;
	...
	if(x_edges.size() < 2 || x_edges.back()->getOverlap().getOverlapLength(0) > m_overlapLen_cutoff)
	    continue;

	//all overlap lengthes are less than cutoff
	//choose the max overlap edge GREEDily
	if(x_edges.front()->getOverlap().getOverlapLength(0) < m_overlapLen_cutoff){
	    for(size_t i = 1; i < x_edges.size(); ++i)
	    {
		   
		Edge* pXY = x_edges[i];		   
		
		//for delete twin edge
		Edge* pYX = pXY->getTwin();
		Vertex* pY = pXY->getEnd();
		//
		
		pX->deleteEdge(pXY);
		pY->deleteEdge(pYX);//delete twin edge
	    ...
	    //keep edges that length exceed CUTOFF
	    for(size_t i = 0; i < x_edges.size(); ++i)
	    {
		Edge* pXY = x_edges[i];
		
		if(pXY->getOverlap().getOverlapLength(0) < m_overlapLen_cutoff)
		{
		    //for delete twin edge
		    Edge* pYX = pXY->getTwin();
		    Vertex* pY = pXY->getEnd();
...
void SGMaxOverlapVisitor::postvisit(StringGraph* /*pGraph*/)
{
}
