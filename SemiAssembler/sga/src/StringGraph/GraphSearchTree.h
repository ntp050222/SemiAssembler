...
int GraphSearchNode<VERTEX,EDGE,DISTANCE>::createChildren(GraphSearchNodePtrDeque& outDeque, const DISTANCE& distanceFunc)
{
	...
	//_EDGEPtrVector edges = m_pVertex->getEdges();
//	std::cout<<"Vertex= "<<m_pVertex->getID()<<"\t Edge="<<edges.size()<<std::endl;
	//_EDGEPtrVector edges = m_pVertex->getEdges();//get all edge include anti
    ...
    for(size_t i = 0; i < edges.size(); ++i)
    {
		...
		//my
		//EdgeDir childExpandDir = correctDir(edges[i]->getDir(), edges[i]->getComp());
		//my
...
bool GraphSearchTree<VERTEX,EDGE,DISTANCE>::stepOnce()
{
    ...
    {
	//adddddddddddd
//	std::cout << "XXX=\t" << m_totalNodes << std::endl ;
        // Move all nodes in the expand queue to the done queue
       // std::cout<<"m_totalNodes="<<m_totalNodes<<std::endl;
...
//            std::cout<<"goal,dis="<<pNode->getDistance()<<"\tm_totalNodes="<<m_totalNodes<<std::endl;
//		std::cout<<"goal, m_totalNodes= "<<m_totalNodes<<std::endl;
        ...
        if(pNode->getDistance() > m_distanceLimit)
        {
			...
			//std::cout<<"pNode="<<m_pGoalVertex<<std::endl;
			//std::cout<<"dis="<<pNode->getDistance()<<std::endl;
             m_doneQueue.push_back(pNode);
        }
        else
        {
            // Add the children of this node to the queue
			//std::cout<<"child,dis="<<pNode->getDistance()<<"m_totalNodes="<<m_totalNodes<<std::endl;
            int numCreated = pNode->createChildren(incomingQueue, m_distanceFunc);
            m_totalNodes += numCreated;
	   
//	    std::cout << "L=\t" << m_totalNodes << std::endl ;
            if(numCreated == 0)
            {
		//std::cout << "L=\t" << m_totalNodes << std::endl ;
...
{
    //adddd
//    std::cout << "GGG=\t" << m_totalNodes << std::endl ;
