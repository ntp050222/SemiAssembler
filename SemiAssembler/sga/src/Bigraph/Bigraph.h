...
class Bigraph
{
    ...
    public:
		...
		//(by yuhan) Rename all the vertices in the graph for MatePair vistor
		void renameVertices_MP(const std::string& prefix = "", std::string StartVertexID="", std::string EndVertexID="", int MpCompFlagS=-1, int MpCompFlagE=-1);

        // Simplify the graph by removing transitive edges
        void simplify();

		//(by yuhan) Simplify the graph by removing transitive edges // for MP vistor
        void simplify_MP(std::string StartVertexID);
		//(by yuhan) Count remaind vertex
		int simplify_CountVertex(); 
    ...
    private:
        ...
        // Simplify the graph by compacting edges in the given direction // for MP (by yuhan)
		void simplify_MP(EdgeDir dir, std::string StartVertexID);
