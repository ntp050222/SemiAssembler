...
void Bigraph::simplify_MP(std::string StartVertexID)
{
	simplify_MP(ED_SENSE,StartVertexID);
	simplify_MP(ED_ANTISENSE,StartVertexID);
}
/*
void Bigraph::simplify_MP(std::string StartVertexID, std::string EndVertexID)
{
    assert(!hasContainment());
    simplify_MP(ED_SENSE, StartVertexID, EndVertexID);
    simplify_MP(ED_ANTISENSE, StartVertexID, EndVertexID);
}
*/
...
// (by yuhan) Simplify the graph by compacting edges in the given direction 
void Bigraph::simplify_MP(EdgeDir dir, std::string StartVertexID)
{
	//std::cout << "simplify: StartVertex:" << StartVertexID << std::endl;
    bool graph_changed = true;
    while(graph_changed)
    {
       // std::cout<<"11111111111111111\n";
        ...
        while(iter != m_vertices.end())
        {
            ...
            if(edges.size() == 1 && !edges.front()->isSelf())
            {
				...
				if( ((iter->second)->getID()==StartVertexID ) && pV2->countEdges(pTwin->getDir()) == 1 &&(iter->second)->countEdges(pSingle->getDir()) == 1)
				{//pV2->countEdges(pTwin->getDir()) == 1 檢查edge是不是只有一條
                   // std::cout<<"22222222222\n";
					//std::cout<< (iter->second)->getID() << "\n";
					//std::cout<< pSingle->getEnd()->getID() << "\n";
					merge(iter->second, pSingle);
					//std::cout<<"22222222222\n";
                    graph_changed = true;
				}
				else if( (pV2->getID()==StartVertexID ) && (iter->second)->countEdges(pSingle->getDir()) == 1&&pV2->countEdges(pTwin->getDir()) == 1)
				{
					//std::cout<<"3333333333\n";
					//std::cout<< pV2->getID() << "\n";
					//std::cout<< pTwin->getEnd()->getID() << "\n";
					merge(pV2, pTwin);
					//std::cout<<"3333333333\n";
                    graph_changed = true;		
				}
                else if (pV2->countEdges(pTwin->getDir()) == 1)
                {//沒有遇到start or end 就照原始SGA的merge
                   // std::cout<<"123456\n";
					//std::cout<< (iter->second)->getID() << "\n";
					//std::cout<< pSingle->getEnd()->getID() << "\n";
					merge(iter->second, pSingle);
...
void Bigraph::simplify_MP(EdgeDir dir, std::string StartVertexID)
{
	//std::cout << "simplify: StartVertex:" << StartVertexID << " ::: EndVertex " << EndVertexID << std::endl;
    bool graph_changed = true;
    while(graph_changed)
    {
        //std::cout<<"11111111111111111\n";
...
int Bigraph::simplify_CountVertex()
{
	int ContigNum=0;
	//std::cout << "simplify: StartVertex:" << StartVertexID << " ::: EndVertex " << EndVertexID << std::endl;
    VertexPtrMapIter iter = m_vertices.begin(); 
...
// (by yuhan) 
void Bigraph::renameVertices_MP(const std::string& prefix, std::string StartVertexID, std::string EndVertexID, int MpCompFlagS, int MpCompFlagE)
{
    size_t currIdx = 0;
    size_t numVertices = m_vertices.size();
    std::vector<Vertex*> vertexPtrVec(numVertices, 0);
    ...
    while(iter != m_vertices.end())
    {
		...
		if ((iter->second)->getID() == StartVertexID)
            ss << (iter->second)->getID() << "_MpCompFlag:" << MpCompFlagS << "_" << prefix << currIdx;
		else if ((iter->second)->getID() == EndVertexID)
            ss << (iter->second)->getID() << "_MpCompFlag:" << MpCompFlagE << "_" << prefix << currIdx;
		else 
        	ss << (iter->second)->getID() << "_MpCompFlag:-1_" << prefix << currIdx;
