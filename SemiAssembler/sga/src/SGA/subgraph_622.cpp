...
//dont know
	...
	//	StringGraph* pSubgraph_all = new StringGraph;
		...
		if(flag_start==0)
			SGSearch::findWalks(pX,pY,ED_SENSE,opt::mean_d+opt::var_d+pX->getSeqLen()+pY->getSeqLen(),opt::LeafCount,false,allwalk);//change 500->opt::LeafCount
		//	SGSearch::findWalks(pX,pY,ED_SENSE,opt::mean_d+opt::var_d,opt::LeafCount,false,allwalk);
		else if(flag_start==1)    
            	        SGSearch::findWalks(pX,pY,ED_ANTISENSE,opt::mean_d+opt::var_d+pX->getSeqLen()+pY->getSeqLen(),opt::LeafCount,false,allwalk);//
		...
		if(allwalk.size()>0)
		{
			...
			*/
			...
			flag_path=0;
			std::cout << "mean=" << opt::mean_d << "\t var=" << opt::var_d << std::endl ;
			...
			/*	if(allwalk[i].getString(SGWT_INTERNAL).size()<=(opt::mean_d+opt::var_d))
				...
				else
				{
					std::cout << "allwalk-" << i << "=" << "\t legal path.  Size="<< allwalk[i].getString(SGWT_INTERNAL).size() << std::endl ;
                                ...
                                goal_path_cmp=(allwalk[i].getString(SGWT_INTERNAL).size())-(opt::mean_d);
