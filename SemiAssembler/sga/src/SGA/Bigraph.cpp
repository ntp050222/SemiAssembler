...
// Simplify the graph by compacting edges in the given direction
void Bigraph::simplify_MP(EdgeDir dir, std::string StartVertexID, std::string EndVertexID)
{
    ...
    while(graph_changed)
    {
        ...
        while(iter != m_vertices.end())
        {
            ...
            if(edges.size() == 1 && !edges.front()->isSelf())
            {
				...
				if( ((iter->second)->getID()==StartVertexID || (iter->second)->getID()==EndVertexID) && pV2->countEdges(pTwin->getDir()) == 1 )
				{
                    merge(iter->second, pSingle);
                    graph_changed = true;
				}
				else if( (pV2->getID()==StartVertexID || pV2->getID()==EndVertexID) && (iter->second)->countEdges(pSingle->getDir()) == 1)
				{
					merge(pV2, pTwin);
                    graph_changed = true;		
				}
