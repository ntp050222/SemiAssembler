...
void subgraph()
{
	...
	for(mateNum=0 ; mateNum < mateNum_up_bound ; mateNum++)  // for each mate pair
	{
		...
		std::vector<std::string> LDFS;//large degree from start
		...
		//	std::cout << "mp" << mateNum+1 << "\n";
		//	std::cout<<"StartVertixID_list.size()="<<StartVertixID_list.size()<<"\n";
			...
			else
			{
				...
				//std::cout << "mp" << mateNum+1 <<"\tLDFS="<<LargeDegreeVertexFromS<<"\n";
		...
		if(Mode_Control==1)  // if Start to End Faild
		{
			...
			if(LDFS.size()!=0)
			{
				std::cout<<"mp"<<mateNum+1<<"\tLDFS.size()="<<LDFS.size()<<"\n";
				for(size_t i=0;i<LDFS.size();i++){
					//std::cout<<"i="<<i<<"\n";
					LargeDegreeVertexFromS=LDFS[i];
					if(LargeDegreeVertexFromS!="")
						End_set_ES->insert(LargeDegreeVertexFromS);
					//std::cout<<"i="<<i<<"\n";				
				}
			}
			else if(LDFS.size()==0)
			...
			{
				//std::cout<<"mp"<<mateNum+1<<"is in step 2, and i is "<<i+1<<"\n";
				//std::cout<<"mp="<< mateNum+1<<"\tLDFS="<<LargeDegreeVertexFromS<<".\n";
				//std::cout<<"EndVertixID_list.size()="<<EndVertixID_list.size()<<"\n";
				...
				if(stop==1)  // break of muti start
				{
					//std::cout<<"mp"<<mateNum+1<<"is break and i is "<<i<<"\n";	
					break;
				}
			}  // for start point
			delete End_set_ES;// by little8
			delete visit_Vertex_set_ES;// by little 8
			//std::cout << "   Step2.VertexCount=" << VertexCount << std::endl;
			VertexCount2=VertexCount; //by little8
		}
		...
		/*	if(LargeDegreeVertexFromS!="")
                        ...
                        if(LDFS.size()!=0)
                        {
                              //  std::cout<<"mp"<<mateNum+1<<"\tLDFS.size()="<<LDFS.size()<<"\n";
                                for(size_t i=0;i<LDFS.size();i++){
                                        ...
                                        LargeDegreeVertexFromS=LDFS[i];
                                        if(LargeDegreeVertexFromS!="")
                                                End_set_SE->insert(LargeDegreeVertexFromS);
                                        //std::cout<<"i="<<i<<"\n";
                                }
                        }
                        else if(LDFS.size()==0)
			...
			VertexCount=0;
			//for(size_t i = 0; i < StartVertixID_list.size(); ++i)    // for each start point // Start to End search
			for(size_t i = 0; i < StartVertixID_list.size(); i++)// i<1 at first
			{
				if(i>3) break;
                        ...
                        if(LDFE.size()!=0)
                        {
                               // std::cout<<"mp"<<mateNum+1<<"\tLDFS.size()="<<LDFS.size()<<"\n";
                                for(size_t i=0;i<LDFE.size();i++){
                                        //std::cout<<"i="<<i<<"\n";
                                        LargeDegreeVertexFromE=LDFE[i];
                                        if(LargeDegreeVertexFromE!="")
                                                End_set_ES->insert(LargeDegreeVertexFromE);
                                        //std::cout<<"i="<<i<<"\n";
                                }
                        }
                        else if(LDFE.size()==0)
